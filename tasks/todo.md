...
- [x] **20. Hard gap between reads** (branch `fix/scan-gap`) — nothing is read within 3 s of the previous read, even a different product, which drops misreads of a moving barcode (e.g. a shorter number right after the good read). `gapMs` knob in `Scanner.kt`. Plus a 3-frame confirmation (`confirmFrames`): a code only counts once read identically in 3 consecutive frames (~0.1–0.2 s), so flickering partial reads never count.
- [x] **21. Multiple code scans setting** (branch `feature/repeat-scan-modes`) — Hard lock (default, previous behaviour: a code must leave view 3 s before it's read again; warn if already in the area), Soft lock (re-read after the 3 s gap even if still in view; always warn), No lock (re-read after the gap; save duplicates silently). The 3 s gap and 3-frame confirmation apply in every mode, and the gap now restarts while a popup or the scans list is open.
